package FileCabinet.Bases.Classes.ParseData;

import FileCabinet.Bases.AbstractClasses.ADataParseProcessor;
import FileCabinet.Bases.Exceptions.FullNameReadFromDataException;
import FileCabinet.Bases.Exceptions.GetDataArrayException;
import FileCabinet.Bases.Exceptions.ParseDataException;
import FileCabinet.Bases.Exceptions.ParseDataToIntegerException;
...
public class CDataParseProcessor extends ADataParseProcessor {
    ...
    /**
     * Парсит строку и раскладывает ее по элементам
        ...
        try {
            for (int i = 0; i < dataArray.length; i++) {
                if (this.dataArray[i].length() == 1)
                    ...
                    try {
                        this.phoneNumber = Long.parseLong(this.dataArray[i]);// Проверка номера телефона
